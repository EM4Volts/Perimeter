...
def perimeter_make_rpak_map( rpak_params, rpak_slots):
    ...
    for rpak_slot in rpak_slots:
        if rpak_slot == "None":
            ...
            if rpak_slot.endswith(".png"):
                normalized_slot_name = rpak_params["rpak_asset_path"] + "/" + os.path.basename(rpak_slot).replace(".png", "")
                normalized_slots.append( normalized_slot_name.replace("//", "/") )
            else:
                normalized_slots.append("")
    ...
    for slot in normalized_slots:
        if slot != "":
            rpak_map_json["files"].append( { "$type":"txtr","path":f'{rpak_params["rpak_asset_path"]}/{os.path.basename(slot)}'.replace("//", "/").replace(".png", "").replace(".jpg", "").replace(".tga", ""),"saveDebugName": True} )
    ...
    files_sub_preset_json["path"] = f'{rpak_params["rpak_asset_path"]}/{rpak_params["rpak_name"]}'
    ...
    files_sub_preset_json["shadaderset"] = rpak_params["shaderset"]
    ...
    json.dump(rpak_map_json, open(f"{json_map_name}.json", "w"), indent=4)